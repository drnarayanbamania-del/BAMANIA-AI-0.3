    ...
    <div className="flex min-h-screen bg-[#030712] overflow-hidden">
      ...
      <main className="flex-1 relative flex flex-col items-center justify-start p-4 lg:ml-80 transition-all overflow-y-auto custom-scrollbar pt-24 pb-32">
        ...
        <div className="w-full max-w-4xl flex flex-col items-center gap-6 mt-8">
          ...
          >
            {/* Skeleton Loader / Loading UI */}
            {(isLoading || isVariationsLoading) && (
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center bg-black/80 overflow-hidden">
                  ...
                  <div className="w-64 space-y-6">
                    <div className="space-y-2">
                       <p className="text-sm font-mono text-blue-400 tracking-widest uppercase">
                         {isVariationsLoading ? 'Dreaming Variations' : 'Processing Neural Grid'}
                       </p>
                       <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                        <div 
                          className="h-full bg-gradient-to-r from-blue-500 to-purple-500 shadow-[0_0_15px_rgba(59,130,246,0.5)] transition-all duration-300" 
                          style={{ width: `${loadingProgress}%` }}
                        ></div>
                      </div>
                      <div className="flex justify-between text-[10px] font-mono text-gray-500">
                        <span>{Math.round(loadingProgress)}% Complete</span>
                        <span>Layer: {loadingStep + 1}/7</span>
                      </div>
                    </div>

                    <div className="h-8 flex items-center justify-center">
                      <p className="text-lg font-medium text-white/90 animate-in fade-in slide-in-from-bottom-2 duration-500" key={loadingStep}>
            ...
            {currentImage ? (
              <img 
                ...
                className={`w-full h-full object-cover transition-all duration-700 ${isLoading || isVariationsLoading ? 'opacity-0 scale-105' : 'opacity-100 scale-100'}`}
              />
            ) : (
              !isLoading && !isVariationsLoading && (
          ...
          {/* Action Button Bar */}
          {currentImage && !isLoading && !isVariationsLoading && (
            <div className="flex flex-wrap items-center justify-center gap-3 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {variations.length > 0 && !isVariationsLoading && (
            <div className="w-full mt-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-sm font-bold uppercase tracking-[0.2em] text-gray-500">Variations</h3>
                <div className="h-px flex-1 bg-white/10"></div>
              </div>
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 z-[120] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
          <div className="glass max-w-sm w-full p-8 rounded-3xl border border-white/10 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-2xl font-bold text-center mb-2">Clear History?</h3>
            <p className="text-gray-400 text-center mb-8">This will permanently delete all your generated creations from this device.</p>
